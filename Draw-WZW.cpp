...
{
	if(min(c,d)<max(a,b)) return 0;
	if(a==b) return min(c,d)-a;
...
{
	scanf("%d",&n);
	per(i,1,n) scanf("%d %d",&a[i][0],&a[i][1]);
	
	int ans=0;
	per(i,1,n){
		//if(i!=1&&a[i][0]==a[i-1][0]&&a[i][1]==a[i-1][1]) continue;
		ans+=solve(a[i-1][0],a[i-1][1],a[i][0],a[i][1]);		
	}
	printf("%d\n",ans+1);
}
