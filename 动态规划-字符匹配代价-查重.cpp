...
void solve(){
	...
	per(i,1,m){
		dp[i%2][0] =i * ctk;dp[(i-1)%2][0] = (i-1) * ctk;
		per(j,1,n){
			//dp[i][j] = min(dp[i-1][j-1] + (s1[i-1] == s2[j-1] ? 0 : mis),min(dp[i-1][j],dp[i][j-1]) + ctk); 
			dp[i%2][j] = min(dp[(i-1)%2][j-1] + (s1[i-1] == s2[j-1] ? 0 : mis),min(dp[(i-1)%2][j],dp[i%2][j-1]) + ctk);
		}
	}
