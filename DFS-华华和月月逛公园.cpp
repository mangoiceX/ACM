...
bool vis[maxn+10];
int cnt = 0;
void dfs(int s,int v1,int v2){
	int size = g[s].size();
	per(i,0,size-1){
		if(s == v1){
			if(g[s][i] != v2 && !vis[g[s][i]]){
				vis[g[s][i]] = true;
				++cnt;
				dfs(g[s][i],v1,v2);
			}
		}else if(s == v2){
			if(g[s][i] != v1 && !vis[g[s][i]]){
				vis[g[s][i]] = true;
				++cnt;
				dfs(g[s][i],v1,v2);
			}
...
int main(){
	...
	while(~scanf("%d %d",&n,&m)){
		per(i,1,m){
			...
			g[a[i].first].push_back(a[i].second);g[a[i].second].push_back(a[i].first);
		}
		int ans = 0;
		per(i,1,m){
			memset(vis,false,sizeof(vis));
			cnt = 0;
			dfs(1,a[i].first,a[i].second);
			if(cnt < n){
				++ans;
			}
		}
